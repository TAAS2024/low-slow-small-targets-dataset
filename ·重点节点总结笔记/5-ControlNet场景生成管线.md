...
## 十三、CN-LoRA 对比微调方案（CDFF v2.0 集成）

> 关联笔记：[[7-持续学习循环设计]] — 统一自训练循环中的 ControlNet 组件

### 13.1 定位

ControlNet 在自训练循环中采用**范式 B：失败引导重生成 + 对比学习**。

原因：ControlNet 核心问题是条件控制精度——同一个 depth/seg map，有时出好图有时出烂图。正样本筛选（范式 A）无法解决「同条件质量波动」问题，对比学习直接告诉模型"同样的条件，要那个不要那个"。

### 13.2 触发条件

```
S0_BG_UNREALISTIC 失败 → 场景类型聚类 → 同一类型占比>50%且≥30条 → 触发CN微调
```

防抖逻辑：均匀随机失败 = prompt/seed 问题，不训模型。只有**系统性场景失败**（如 urban+overcast 类型反复崩塌）才触发。

### 13.3 训练流程

```
1. CN-Buffer 攒满30条同类型失败样本
       │
2. 提取每条样本的 seg map 条件图 + 原始 prompt
       │
3. 对每条：保持 seg map 不变，调整 prompt（增加场景细节描述）
   → 重生成 → S0 验证
       │
   ├─ 修复成功 → 形成对比 pair: (seg_map, 失败图) vs (seg_map, 修复图)
   └─ 修复失败 → 丢弃（可能是 seg map 本身的问题）
       │
4. 收集 ≥15 对有效对比 pair
       │
5. CN-LoRA 对比微调
   - 冻结 SD1.5 UNet
   - 仅在 ControlNet 编码器层注入 LoRA (rank=4, alpha=2)
   - 学习率 1e-6，max 200步
   - 损失：MSE(去噪 latent, 目标 latent) + λ × 对比正则项
       │
6. A/B 评估: 微调前后同条件生成20张 → 过S0-S3 → 对比 failure rate
       │
7. 效果提升 → 替换线上 ControlNet 权重
   效果退化 → 回滚，记录原因
```
...
| 参数 | 值 | 理由 |
|:--|:--|:--|
| LoRA rank | 4 | CN 只做小幅修正，不需大容量 |
| LoRA alpha | 2 | alpha=rank/2，温和注入 |
| 学习率 | 1e-6 | 极低 lr，避免破坏原 CN 分割理解能力 |
| 最大步数 | 200 | 数据集极小（~15对），早停防过拟合 |
| 注入位置 | CN 编码器层 | 冻结 UNet，仅调 CN 条件注入 |
| 最小有效 pair | 15 | 低于此数不训，防止噪声 |

### 13.5 论文定位

CN-LoRA 对比微调是 CDFF 闭环数据飞轮的第二环（第一环 LoRA，第三环 IR CNN）：

```
验证失败 → 定位责任组件 → 生成修复数据 → 对比微调 → 替换权重 → 下一轮失败率下降
```

可论证：**ControlNet 的生成质量在闭环反馈下系统性提升**（消融：仅LoRA vs +CN对比微调 → S0 failure rate 对比）。
